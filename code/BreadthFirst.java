...
public class BreadthFirst implements Strategy {
	boolean visualize;
	...
	public BreadthFirst(String grid,boolean visualize) {
	...
	public String solve() {
		...
		while (!queue.isEmpty()) {
			...
			if (s.done() && !s.Dead() && BestSolution == null && s.row==TelephoneRow && s.col == TelephoneCol) {
				 ...
				 * "up,left,carry,down,down,right,down,carry,kill,down,right,carry,left,left,left,left,drop"
				 * )) { System.out.println(s.plan); System.out.println(s.row+","+s.col);
				 * 
				 * System.out.println(s.carriedHostages.toString());
				 * System.out.println(s.hostageLocs.toString());
				 * System.out.println(s.deadHostages.toString());
				 *  
				 * System.out.println(s.capacity); return"hi"; }
				 */
				...
				if (!found) {
					...
					if (s.CanKill() && s.LegalMoveKill()) {
						nTemp = new Node(s,s.row, s.col, s.Damage, s.capacity, s.carriedHostages,
								s.carriedHostagesDamage, s.deadHostages, s.agentLocs, s.pillLocs, s.hostageLocs,
								s.hostageDamages,0, s.cost + 1000, s.plan, s.deaths, s.kills);
					...
					if (IsTherePortal(s) && IsRelevantMove(s, "fly") && !IsRedundantFly(s)) {
						...
						nTemp = new Node(s,Integer.parseInt(newLocArr[0]), Integer.parseInt(newLocArr[1]), s.Damage,
								...
								s.pillLocs, s.hostageLocs, s.hostageDamages,0, s.cost + 10, s.plan + "fly,", s.deaths,
								s.kills);
						nTemp.state.ApplyTime();
					...
					if (s.row > 0 && !s.IsThereAgentAbove() && IsRelevantMove(s, "up") && s.LegalMoveUp()) {
					...
					if (s.row < maxRow && !s.IsThereAgentUnder() && IsRelevantMove(s, "down") && s.LegalMoveDown()) {
						nTemp = new Node(s,s.row + 1, s.col, s.Damage, s.capacity, s.carriedHostages,
								s.carriedHostagesDamage, s.deadHostages, s.agentLocs, s.pillLocs, s.hostageLocs,
					...
					if (s.col > 0 && !s.IsThereAgentLeft() && IsRelevantMove(s, "left") && s.LegalMoveLeft()) {
					...
					if (s.col < maxCol && !s.IsThereAgentRight() && IsRelevantMove(s, "right") && s.LegalMoveRight()) {
						nTemp = new Node(s,s.row, s.col + 1, s.Damage, s.capacity, s.carriedHostages,
								s.carriedHostagesDamage, s.deadHostages, s.agentLocs, s.pillLocs, s.hostageLocs,
